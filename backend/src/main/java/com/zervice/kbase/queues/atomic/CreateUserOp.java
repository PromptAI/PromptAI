...
public class CreateUserOp {
    ...
    public static User createUser(Connection conn, AccountCatalog account, RestUser userRest, boolean quick) throws Exception {
        ...
        if (!StringUtils.isEmpty(lowerEmail)) {
            ...
            if (acct2 != null) {
                LOG.error("[{}][User email:{} already exists in another account:{}]", dbName, lowerEmail, acct2.getName());
        ...
        if (!StringUtils.isEmpty(mobile)) {
            if (account.findUserByMobile(mobile) != null) {
                throw new RestException(StatusCodes.MOBILE_ALREADY_EXISTS, "The user mobile already exists - " + mobile);
            }

            AccountCatalog acct2 = AccountCatalog.getUserAccountByMobile(mobile);
            if (acct2 != null) {
                LOG.error("[{}][User mobile:{} already exists in another account:{}]", dbName, mobile, acct2.getName());
                throw new RestException(StatusCodes.MOBILE_ALREADY_EXISTS, "The user mobile already token - " + mobile);
            }
        }
        String email = userRest.getEmail();
        if (!StringUtils.isEmpty(email)) {
            if (account.findUserByEmail(email) != null) {
                throw new RestException(StatusCodes.EMAIL_ALREADY_EXISTS, "The user email already exists - " + mobile);
            }

            AccountCatalog acct2 = AccountCatalog.getUserAccountByMobile(mobile);
            if (acct2 != null) {
                LOG.error("[{}][User email:{} already exists in another account:{}]", dbName, mobile, acct2.getName());
                throw new RestException(StatusCodes.EMAIL_ALREADY_EXISTS, "The user email already token - " + mobile);
            }
        }
