...
public class AccountActivationOp {
    ...
    public static void process(AtomicTask task) {
        ...
        if (account == null) {
            LOG.error("[{}][Activating invalid account. acctId:{}]", dbName, acctId);
        ...
        if (admins == null || admins.size() == 0) {
            ...
            LOG.error("[{}][Empty admin list for account. accId:{}]", dbName, acctId);
        ...
        try {
            ...
            if (quickMode) {
                activateAccountQuick(account, conn, adminEmail, adminMobile, adminName);
            } else {
    ...
    public static void activateAccount(AccountCatalog account, Connection conn, JSONArray admins, JSONArray users) throws Exception {
        ...
        LOG.info("[{}]Creating users for {}", Constants.COREDB, account.getName());
    ...
    public static User activateAccountQuick(AccountCatalog account, Connection conn, String adminEmail, String adminMobile, String username) throws Exception {
        // get the roles for admin
        RbacRole rr = RbacRoleDao.get(conn, account.getDBName(), RBACConstants.SYS_ADMIN);
        Set<RestRole> adminRoles = new HashSet<>(Collections.singletonList(new RestRole(rr)));

        // for each user, queue a CreateUserCo
        LOG.info("[{}][Quick creating users for account:{}, email:{}, mobile:{} ]", Constants.COREDB, account.getName(), adminEmail, adminMobile);

        // the admin phone # shall be correct
        return _createUserSync(conn, account, username, adminEmail, adminMobile, adminRoles);
    }
    ...
    private static void _createUsers(Connection conn, AtomicTaskQueues atomicTaskQueues, long acctId, JSONArray users, Set<RestRole> roles) throws Exception {
        for (int i = 0; i < users.size(); i++) {
            ...
            LOG.info("[{}][Queue user creation for user:{}]", Constants.COREDB, email);
     ...
     */
    private static User _createUserSync(Connection conn, AccountCatalog account, String name, String email, String mobile, Set<RestRole> roles) throws Exception {
        Preconditions.checkArgument(!StringUtils.isEmpty(email) || !StringUtils.isEmpty(mobile));
        LOG.info("[{}][Queue quick synchronous user creation, emial:{}, mobile:{}]", Constants.COREDB, email, mobile);
