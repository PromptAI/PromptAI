...
import { infoWebhook, updateWebhook } from '@/api/components';
...
const Webhook = () => {
  ...
  const { wId, id: projectId } = useParams<{ wId: string; id: string }>();
  ...
  // 如果是 form，将 body 转换为数组
  function parseRequestBody(values: any) {
    const { request_body_type, request_body } = values;
    // 将 json 转换成数组
    if (request_body_type === 'multipart/form-data') {
      return request_body ? Object.entries(JSON.parse(webhook.request_body)).map(([key, value]) => ({key, value})) : [];
    }

    return request_body;
  }

  const { loading, data: webhook } = useRequest(
    () => infoWebhook(projectId, wId),
    {
      refreshDeps: [projectId, wId],
      onSuccess: () => {
        const wt = webhook.data?.webhookType;
        const code= webhook.data?.code;
        setWebhookType(wt);
        if (wt === 'python_code'){
          setCode(code);
        ...
        form.setFieldsValue({
          ...webhook,
          webhook_type: wt,
          request_body: parseRequestBody(webhook),
          url: {
            agreement,
            api,
          },
  ...
  useUpdateEffect(() => {
    if (!loading && webhook) {
      setResponseType(webhook.response_type);
      setRequestHeaderType(webhook.request_header_type);
    }
  }, [loading, webhook]);
    ...
    form
      ...
      .then((values) => {
        ...
        if (webhookType === 'python_code') {
          return updateWebhook(projectId, wId, {
            ...values,
      ...
      .catch((error) => {
        ...
        Message.error(dt['message.update.failure']);
      })
      .finally(() => {
        setSubmiting(false);
      });
  ...
  function WebRequestView() {
    return <>
      ...
      <Form.Item
        label={t['webhooks.form.request-header-type']}
