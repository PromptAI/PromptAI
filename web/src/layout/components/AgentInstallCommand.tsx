...
export const CopyButton: React.FC<CopyButtonProps> = ({ text }) => {
  ...
  const onCopy = (_, result) => {
    if (result) {
      Message.success(t['agent.copy.success']);
    }
  };
  return (
  ...
  return (
    <div style={{ padding: '0px 16px 16px' }}>
      <Typography.Title heading={5}>
        {t['agent.install.requirements.title']}
      </Typography.Title>
        ...
        >
          <ol>
            {requirements.map((_, i) => (
              <li key={i}>{t[`agent.install.requirements.${i}`]}</li>
            ))}
          </ol>
        </Typography.Paragraph>
      </div>
...
const AgentInstallCommand: React.FC<AgentInstallCommandProps> = ({ cmd }) => {
  return (
...
export default AgentInstallCommand;
