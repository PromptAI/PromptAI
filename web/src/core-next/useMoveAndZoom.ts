...
class MoveAndZoom<T extends HTMLElement> {
  ...
  static newInstance<T extends HTMLElement>(config: Config) {
    let instance: MoveAndZoom<T> = undefined;
    if (!instance) {
      instance = new MoveAndZoom<T>(config);
    }
    return instance;
  }
...
const INSTANCE = MoveAndZoom.newInstance({
  contrlMoving: false,
  unMovingClassNames: ['node', 'mind-graph-node-wrapper'],
});
export default function useMoveAndZoom<T extends HTMLElement>(
  actionRef?: MutableRefObject<T>
) {
  const ref = useRef<T>();
  useLayoutEffect(() => {
    INSTANCE.initial(ref.current, actionRef?.current);
    return () => {
      INSTANCE.destroy();
    };
...
export function useZoom() {
  ...
  useLayoutEffect(() => {
    INSTANCE.on('zoom', setZoom);
  }, []);
  return [
    zoom,
    INSTANCE.zoomIn.bind(INSTANCE),
    INSTANCE.zoomOut.bind(INSTANCE),
  ] as [number, () => void, () => void];
}
